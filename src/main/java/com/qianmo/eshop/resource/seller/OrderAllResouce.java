...
public class OrderAllResouce extends SellerResource {
    ...
    @GET
    public HashMap getList(String buyer_name_num,String data_end,String data_start ,int date_step, Integer order_status,Integer page_start,Integer page_step) {
        ...
        for (order_user or_numlist : order_userList) {
            ...
            if (date_step == ConstantsUtils.ORDER_TIME_TODAY){  //1:表示今天
                sql1_1 = sql1_1 + "and  date(ou.created_at) = date(sysdate())";
                sql1_2 = sql1_2 + " and date(oi.created_at) = date(sysdate())";
                sql3 = sql3 + " and date(oi.created_at) = date(sysdate())";
                sql4 = sql4 + " and date(order_remark.created_at) = date(sysdate())";
                order_user_sql = order_user_sql + "and date(ou.created_at) = date(sysdate())";

            }else if (date_step == ConstantsUtils.ORDER_TIME_F){//   7  表示近7天
                sql1_1 = sql1_1 + "and  date(ou.created_at) > date_add(date(sysdate()), interval -6 day)";
                sql1_2 = sql1_2 + " and date(oi.created_at) > date_add(date(sysdate()), interval -6 day)";
                sql3 = sql3 + " and date(oi.created_at) >date_add(date(sysdate()), interval -6 day)";
                sql4 = sql4 + " and date(order_remark.created_at) > date_add(date(sysdate()), interval -6 day)";
                order_user_sql = order_user_sql + "and date(ou.created_at) > date_add(date(sysdate()), interval -6 day)";
            }else if(date_step == ConstantsUtils.ORDER_TIME_s){
                sql1_1 = sql1_1 + "and  date(ou.created_at) > date_add(date(sysdate()), interval -29 day)";
                sql1_2 = sql1_2 + " and date(oi.created_at) > date_add(date(sysdate()), interval -29 day)";
                sql3 = sql3 + " and date(oi.created_at) >date_add(date(sysdate()), interval -29 day)";
                sql4 = sql4 + " and date(order_remark.created_at) > date_add(date(sysdate()), interval -29 day)";
